...
<body>
<h2 style="text-align:center;margin:10px">用户信息列表</h2>
<div class="layui-container" style="margin-left:-35px">
<div class="layui-form layui-form-pane">
 <div class="layui-inline">
    <label class="layui-form-label" for="name">姓名</label>
    <div class="layui-input-inline">
      <input type="text" name="name" id="name" style="width:100px" required  lay-verify="required" placeholder="请输入姓名" autocomplete="off" class="layui-input">
    </div>
  </div>
  <div class="layui-inline">
    <label class="layui-form-label" for="adress">地址</label>
    <div class="layui-input-inline">
    ...
    <div class="layui-input-inline">
      <input type="text" name="email" id="email" style="width:100px" required  lay-verify="required" placeholder="请输入邮箱" autocomplete="off" class="layui-input">
    </div>
    </div>
...
<script type="text/html" id="toolbarDemo">
  <div class="layui-btn-container">
    <button class="layui-btn layui-btn-danger layui-btn-sm" lay-event="delChosedUser">删除选中</button>    
    <button class="layui-btn layui-btn-normal layui-btn-sm" lay-event="addUser">添加用户</button>    
  </div>
</script>
...
layui.use(['jquery','table', 'dropdown'], function(){
  ...
  table.render({
    ...
    //,url:'${pageContext.request.contextPath }/json/users.json' // 此处为静态模拟数据，实际使用时需换成真实接口
        ,url:'${pageContext.request.contextPath }/listUserServlet'
    ,toolbar: '#toolbarDemo'
    ...
    ,cols: [[
      ...
      ,{field:'address', width:115, title: '地址'}
      ,{field:'qq', title: 'QQ',  Width: 160}
      ,{field:'email', title:'邮箱 <i class="layui-icon layui-icon-email"></i>', hide: 0, edit: 'text'}       
      ...
      case 'delChosedUser':
    	  ...
    	  if(selects.length!=0){
    		  layer.confirm('真的删除选中记录吗？', function(index){
    		        ...
    		        $.post("${pageContext.request.contextPath }/delChoseUserServlet",{"ids":ids},function(res){
    		        	//console.log("res："+res);
    		        	//layer.alert(res);
      ...
      break;
    	  ...
    	  //layer.msg("添加用户");//打开添加界面
    	   layer.open({
        		...
        		area: ['900px', '600px'],
        		content: '${pageContext.request.contextPath }/user/addUser.jsp',
        		end: function(){
        			setTimeout("layui.table.reload('userTable')",200);  //刷新列表
        		  }
      			});
  ...
  table.on('tool(test)', function(obj){ // 双击 toolDouble
    ...
    if(obj.event === 'del'){
      layer.confirm('真的删除行么', function(index){
    	  $.post("${pageContext.request.contextPath }/delUserServlet",{"id":data.id},function(res){
    	      console.log(res);
      ...
      //显示被删除记录的主键
        ...
        //layer.alert(data.id);
        //$.post("${pageContext.request.contextPath }/getUserServlet",{"id":data.id},function(res){
        //	
        //});
    	layer.open({
    		type: 2,
    		...
    		maxmin: true, //开启最大化最小化按钮
    		area: ['900px', '600px'],
    		content: '${pageContext.request.contextPath }/getUserServlet?id='+data.id,
    		end: function(){
    			console.log("end");
    			 setTimeout("layui.table.reload('userTable')",200);  //刷新列表
    		  }
